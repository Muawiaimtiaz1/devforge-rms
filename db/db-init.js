...
await query("ALTER TABLE sales ADD COLUMN IF NOT EXISTS order_number INTEGER");
    ...
    `);
    await query("CREATE INDEX IF NOT EXISTS idx_kitchen_order_updates_shop_id ON kitchen_order_updates(shop_id)");
    await query(`
    ...
    `);
    await query("CREATE INDEX IF NOT EXISTS idx_kitchen_pending_updates_shop ON kitchen_order_pending_updates(shop_id, kitchen_id, updated_at)");
    await query(`
    ...
    for (const columnName of ['variants_config', 'addons_config']) {
      const columnCheck = await query(`
        SELECT column_name FROM information_schema.columns
        WHERE table_name = 'products' AND column_name = $1
      `, [columnName]);
      if (columnCheck.rows.length === 0) {
        await query(`ALTER TABLE products ADD COLUMN ${columnName} JSONB NOT NULL DEFAULT '[]'::jsonb`);
      }
    }
    const productTypeCheck = await query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'products' AND column_name = 'product_type'
    `);
    if (productTypeCheck.rows.length === 0) {
      await query("ALTER TABLE products ADD COLUMN product_type TEXT NOT NULL DEFAULT 'stock_based'");
    }
    await query(`
      CREATE TABLE IF NOT EXISTS product_stock_variants (
        ...
        is_on_menu BOOLEAN NOT NULL DEFAULT FALSE,
    ...
    const stockVariantActiveCheck = await query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'product_stock_variants' AND column_name = 'is_active'
    `);
    if (stockVariantActiveCheck.rows.length === 0) {
      await query('ALTER TABLE product_stock_variants ADD COLUMN is_active BOOLEAN NOT NULL DEFAULT TRUE');
    }
    const saleStockVariantCheck = await query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name = 'sale_items' AND column_name = 'stock_variant_id'
    `);
    if (saleStockVariantCheck.rows.length === 0) {
      await query('ALTER TABLE sale_items ADD COLUMN stock_variant_id INTEGER REFERENCES product_stock_variants(id)');
    }
    await query('CREATE INDEX IF NOT EXISTS idx_sale_items_stock_variant_id ON sale_items(stock_variant_id)');
    await query(`
      INSERT INTO product_stock_variants
        (shop_id, product_id, name, sku, barcode, buying_price, selling_price, stock, min_stock_level, is_default, is_on_menu)
      SELECT p.shop_id, p.id, 'Regular', p.sku, p.barcode, p.buying_price, p.selling_price, p.stock, p.min_stock_level, TRUE, TRUE
      FROM products p
      WHERE COALESCE(p.product_type, 'stock_based') = 'stock_based'
        AND COALESCE(p.is_deleted, 0) = 0
        AND COALESCE(p.is_component, 0) = 0
        AND NOT EXISTS (SELECT 1 FROM product_stock_variants v WHERE v.product_id = p.id)
    ...
    await query("CREATE UNIQUE INDEX IF NOT EXISTS idx_menu_addons_shop_name ON menu_addons(shop_id, LOWER(name))");
    await query(`CREATE TABLE IF NOT EXISTS menu_addon_ingredients (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      menu_addon_id INTEGER NOT NULL REFERENCES menu_addons(id) ON DELETE CASCADE,
      raw_stock_id INTEGER NOT NULL REFERENCES raw_stocks(id) ON DELETE CASCADE,
      quantity DOUBLE PRECISION NOT NULL,
      UNIQUE (menu_addon_id, raw_stock_id)
    )`);
    await query("CREATE INDEX IF NOT EXISTS idx_menu_addon_ingredients_addon_id ON menu_addon_ingredients(menu_addon_id)");
    await query(`INSERT INTO menu_addon_ingredients (menu_addon_id, raw_stock_id, quantity)
      SELECT id, raw_stock_id, quantity FROM menu_addons
      WHERE raw_stock_id IS NOT NULL AND quantity > 0
      ON CONFLICT (menu_addon_id, raw_stock_id) DO NOTHING`);
