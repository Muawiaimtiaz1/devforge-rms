...
CREATE INDEX IF NOT EXISTS idx_staff_access_audit_profile_created ON staff_access_audit(staff_profile_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_staff_access_audit_shop_created ON staff_access_audit(shop_id, created_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_session_devices_device_id ON session_devices(device_id);

CREATE INDEX IF NOT EXISTS idx_session_devices_user_active ON session_devices(user_id, revoked_at, last_seen_at DESC);
...
CREATE TABLE IF NOT EXISTS products (
  ...
  variants_config JSONB NOT NULL DEFAULT '[]'::jsonb,
  addons_config JSONB NOT NULL DEFAULT '[]'::jsonb,
  product_type TEXT NOT NULL DEFAULT 'stock_based',
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_shop_order_number
  ON sales(shop_id, order_number) WHERE order_number IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS sale_items (
  ...
  variants_json TEXT,
  addons_json TEXT,
  stock_variant_id INTEGER REFERENCES product_stock_variants(id),
  third_party_person_id INTEGER REFERENCES third_party_persons(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_menu_addons_shop_name ON menu_addons(shop_id, LOWER(name));
...
CREATE INDEX IF NOT EXISTS idx_sale_inventory_consumptions_sale_item ON sale_inventory_consumptions(shop_id, sale_item_id);
CREATE INDEX IF NOT EXISTS idx_sale_inventory_consumptions_batch ON sale_inventory_consumptions(shop_id, raw_stock_batch_id);
...
CREATE INDEX IF NOT EXISTS idx_sale_items_stock_variant_id ON sale_items(stock_variant_id);
