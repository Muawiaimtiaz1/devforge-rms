    ...
    container.innerHTML = `
      <div class="space-y-5">
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div class="rounded-2xl border border-indigo-100 bg-indigo-50 p-5 dark:border-indigo-900 dark:bg-indigo-950/30"><div class="text-[10px] font-black uppercase tracking-widest text-indigo-500">Total received</div><div class="mt-2 text-3xl font-black text-indigo-950 dark:text-white">${registerPaymentMoney(paymentData.summary?.total_amount)}</div><div class="mt-2 text-xs font-bold text-emerald-600">Tips collected: ${registerPaymentMoney(paymentData.summary?.total_tips)} (included)</div></div>
...
function renderRegisterPaymentRows(rows) {
  return rows.length ? rows.map(row => `<tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40"><td class="px-5 py-4"><div class="font-black text-slate-900 dark:text-white">Order #${row.order_id}</div><div class="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">${escapeOrderValue(row.order_status || '—')}${row.customer_name ? ` · ${escapeOrderValue(row.customer_name)}` : ''}</div></td><td class="px-5 py-4 text-sm font-bold text-slate-600 dark:text-slate-300">${registerPaymentContext(row)}</td><td class="px-5 py-4"><span class="inline-flex rounded-lg bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-slate-600 dark:bg-slate-800 dark:text-slate-300">${escapeOrderValue(row.payment_method)}</span></td><td class="px-5 py-4 text-xs font-bold text-slate-500">${registerPaymentDate(row.payment_time)}</td><td class="px-5 py-4 text-right text-sm font-black text-indigo-700 dark:text-indigo-300">${registerPaymentMoney(row.payment_amount)}<div class="mt-1 text-xs font-medium text-emerald-600">Tip: ${registerPaymentMoney(row.tip_amount)}</div></td></tr>`).join('') : `<tr><td colspan="5" class="px-6 py-16 text-center"><div class="font-black text-slate-700 dark:text-slate-200">No received payments found</div><p class="mt-2 text-sm text-slate-400">Try another shift or clear the filters.</p></td></tr>`;
}
