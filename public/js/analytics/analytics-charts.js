...
function renderBarChart(containerId, dataPoints) {
  ...
  const barsHtml = dataPoints.map((dp, idx) => {
    ...
    const formattedSales = new Intl.NumberFormat('en-IN', { style: 'currency', currency: shopCurrencyCode(), maximumFractionDigits: 0 }).format(dp.sales);
...
function renderDonutChart(containerId, slices, totalValue) {
  ...
  const donutSlicesHtml = slices.map((s, idx) => {
    ...
    const formattedSales = new Intl.NumberFormat('en-IN', { style: 'currency', currency: shopCurrencyCode(), minimumFractionDigits: 0, maximumFractionDigits: 2 }).format(s.sales);
  ...
  const formattedTotal = new Intl.NumberFormat('en-IN', { style: 'currency', currency: shopCurrencyCode(), minimumFractionDigits: 0, maximumFractionDigits: 2 }).format(totalValue);
