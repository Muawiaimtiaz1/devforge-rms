              ...
              <div data-inventory-search="${escapeWasteValue(`${product.name} ${product.category} ${(product.stock_variants || []).map(v => `${v.name} ${v.sku} ${v.barcode || ''}`).join(' ')}`.toLowerCase())}" class="inventory-catalog-item bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <div class="mt-3 space-y-2">${(product.stock_variants || []).map(variant => `
                  <div id="inventory-variant-${variant.id}" class="hidden p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700">
                    <div class="flex justify-between gap-3"><div><strong class="text-sm text-slate-900 dark:text-white">${escapeWasteValue(variant.name)}</strong><div class="text-[10px] text-slate-400">${escapeWasteValue(variant.sku)}${variant.barcode ? ` · ${escapeWasteValue(variant.barcode)}` : ''}</div></div><div class="text-right"><strong class="text-sm ${Number(variant.stock) <= Number(variant.min_stock_level) ? 'text-rose-500' : 'text-emerald-600'}">${Number(variant.stock)} in stock</strong><div class="text-[10px] text-slate-400">Cost ${shopCurrencyCode()} ${Number(variant.buying_price).toLocaleString()} · Sell ${shopCurrencyCode()} ${Number(variant.selling_price).toLocaleString()}</div></div></div>
    ...
    <div class="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] p-8 shadow-2xl border border-slate-200 dark:border-slate-800">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="sm:col-span-2"><div class="flex items-center justify-between mb-2"><label class="text-[10px] font-black text-slate-400 uppercase">Batch Expiry Dates</label><span class="text-[10px] text-slate-400">Optional</span></div>
          <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700"><table class="w-full min-w-[520px] text-left text-xs"><thead class="bg-slate-50 dark:bg-slate-800 text-slate-500"><tr><th class="px-3 py-2">Batch</th><th class="px-3 py-2">Remaining</th><th class="px-3 py-2">Buying Price</th><th class="px-3 py-2">Expiry</th></tr></thead><tbody class="divide-y divide-slate-100 dark:divide-slate-800">${(stock.batches || []).map(batch => `<tr><td class="px-3 py-2 font-bold">#${Number(batch.id)}</td><td class="px-3 py-2">${Number(Number(batch.quantity).toFixed(3))} ${escapeWasteValue(stock.unit)}</td><td class="px-3 py-2">${shopCurrencyCode()} ${Number(batch.buying_price || 0).toLocaleString()}</td><td class="px-3 py-2"><input data-batch-expiry="${Number(batch.id)}" type="date" value="${batch.expiry_date ? escapeWasteValue(String(batch.expiry_date).slice(0, 10)) : ''}" class="w-full px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 font-bold" /></td></tr>`).join('') || '<tr><td colspan="4" class="px-3 py-6 text-center text-slate-400">No active stock batches.</td></tr>'}</tbody></table></div>
        </div>
      </div>
...
function kdsPunchedBy(order) {
  ...
  return escapeWasteValue((order.order_type === 'takeaway' ? waiter : '') || fullName || username || waiter || (order.punched_by_user_id ? `User #${order.punched_by_user_id}` : 'Staff'));
}
...
function renderKDSWorkCard(order) {
  const isCompleted = ['ready', 'served', 'completed'].includes(order.order_status);
  const completedLabel = order.order_status === 'served'
  ...
  return `<article class="rounded-2xl border ${isCompleted ? 'border-emerald-200 dark:border-emerald-900/50' : 'border-blue-200 dark:border-blue-900/50'} bg-white dark:bg-slate-900 p-4 shadow-sm">
    <div class="flex justify-between items-start gap-3"><div><div class="font-black text-slate-900 dark:text-white">Order #${displayOrderNumber}</div><div class="text-xs font-bold text-slate-500">${kdsOrderContext(order)}</div><div class="mt-1 text-[10px] font-black text-slate-400">${order.order_type === 'takeaway' ? 'Order taker' : 'Punched by'} ${kdsPunchedBy(order)}</div><span class="inline-flex mt-2 px-2 py-1 h-fit rounded-lg text-[9px] font-black uppercase ${isCompleted ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}">${isCompleted ? completedLabel : 'Preparing'}</span></div>${kdsOrderTimer(order)}</div>
...
function formatWasteMoney(value) {
  ...
  return `${shopCurrencyCode()} ${Number(value || 0).toFixed(2)}`;
}
