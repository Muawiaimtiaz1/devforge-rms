...
function renderShiftReceiptPage(details, options = {}) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Shift ${shift.id} Z Report</title><style>
    ...
    ${row('Tips collected', `${currency} ${money(summary.total_tips)}`)}
    ...
    ${row('Opening cash', `${currency} ${money(summary.opening_balance)}`)}
    ${row('Verified cash drops', `- ${currency} ${money(summary.cash_drops)}`)}
    ...
    ${row('EXPECTED CASH', `${currency} ${money(summary.expected_balance)}`, true)}
    ${row('ACTUAL CASH', `${currency} ${money(shift.closing_balance)}`, true)}
