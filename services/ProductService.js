...
class ProductService {
  async validateMenuOptions(trx, variants, addons, shopId) {
    ...
    const rawStockIds = [...new Set([
      ...
      ...addonList.flatMap(addon => addon.ingredients?.length
        ? addon.ingredients.map(ingredient => Number(ingredient.raw_stock_id))
        : (addon.raw_stock_id ? [Number(addon.raw_stock_id)] : []))
