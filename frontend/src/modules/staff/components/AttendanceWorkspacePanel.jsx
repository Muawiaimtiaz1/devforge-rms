...
import ShiftAttendanceRegister from './ShiftAttendanceRegister'
...
export default function AttendanceWorkspacePanel({ has, onClose, embedded = false }) {
  ...
  const canMarkDaily = has('attendance.mark_daily'); const tabs = [['overview', 'Overview'], ...(canMarkDaily ? [['shift-attendance', 'Shift attendance']] : []), ...(canManage ? [['schedules', 'Schedules']] : []), ...(canCorrect || canApprove ? [['corrections', 'Corrections']] : []), ...(canApprove ? [['snapshots', 'Payroll snapshots']] : [])]
  const content = <div className="attendance-workspace">{embedded && <header className="embedded-panel-heading"><div><p className="section-label">Time operations</p><h2>Scheduling & attendance</h2><p>Authorized shift attendance, clock-out control, schedules, reports, and audited corrections.</p></div></header>}<nav>{tabs.map(([value, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}</button>)}</nav>{error && <div className="form-error">{error}</div>}{tab === 'overview' && <AttendanceCalendar staff={staff} shifts={templates} canManage={canManage || canApprove} />}{tab === 'shift-attendance' && canMarkDaily && <ShiftAttendanceRegister />}{tab === 'schedules' && canManage && <div className="attendance-config"><ShiftTemplateManager templates={templates} onChanged={load} /><WeeklyScheduleEditor staff={staff} templates={templates} /><HolidayManager /></div>}{tab === 'corrections' && (canCorrect || canApprove) && <AttendanceCorrectionsPanel staff={staff} canApprove={canApprove} />}{tab === 'snapshots' && canApprove && <AttendanceSnapshotPanel />}{!embedded && <footer><button className="secondary-button" onClick={onClose}>Close</button></footer>}</div>
