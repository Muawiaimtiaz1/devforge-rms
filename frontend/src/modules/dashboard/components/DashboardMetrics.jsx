...
const ITEMS = (data) => [
  ['Gross Sales', `${getShopCurrency()} ${money(data.summary?.grossSales)}`, `${integer(data.totalSales)} completed order${Number(data.totalSales) === 1 ? '' : 's'}`, 'blue', 'Completed order totals before subtracting return refunds.'],
  ...
  ['Net Revenue', `${getShopCurrency()} ${money(data.totalRevenue)}`, 'Gross sales minus refunds', 'blue', 'Completed-order sales less refunds recorded in the selected period.'],
