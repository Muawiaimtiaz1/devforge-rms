...
function StockVariantRow({ productId, variant, canEdit, onToggleMenu }) {
  ...
  return <tr className="stock-variant-row"><td><strong>{variant.name}</strong><small>{variant.sku}{variant.barcode ? ` · ${variant.barcode}` : ''}</small></td><td><strong className={low ? 'stock-low' : 'stock-ok'}>{number(variant.stock)} in stock</strong><small>Minimum {number(variant.min_stock_level)}</small></td><td><strong>{getShopCurrency()} {money(variant.buying_price)}</strong><small>Buying price</small></td><td><strong>{getShopCurrency()} {money(variant.selling_price)}</strong><small>Selling price</small></td><td><span className={variant.is_on_menu ? 'menu-status published' : 'menu-status'}>{variant.is_on_menu ? 'On menu' : 'Not published'}</span></td><td>{canEdit && <button type="button" className={variant.is_on_menu ? 'variant-menu-button remove-menu' : 'variant-menu-button publish-menu'} onClick={() => onToggleMenu(productId, variant.id, !variant.is_on_menu)}>{variant.is_on_menu ? 'Remove from Menu' : 'Publish to Menu'}</button>}</td></tr>
}
